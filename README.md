...
Experiment journal: `Benchmarks/experiments-log.md` (ЭКСП-001..017) — honest logs including null results.
...
- **GRAMMAR-BIRTH (ЭКСП-015):** successful composite formulas are elevated to new grammar operators (`B{hash} => floor(rad2deg)`). The grammar evolves bottom-up from verified discoveries — not top-down from an architect (unlike Koza ADF).
- **Cultural propagation (ЭКСП-012/013/014):** operators gain weight after successful use; spawn inherits weighted mutations. Knowledge sharing confirmed on composite structures (×2.1 median, ЭКСП-014 final).
...
## Stage status (06.08.2026)

| Stage | Status |
|-------|--------|
| 0 — Reliable invariant extraction | ~85% (11/13 arithmetic tasks, Auto MPG, Wine, honest NOISE refusals) |
| 1 — Living population | mechanisms wired, 24h integration run on laptop |
| 2 — Understanding | stories mapped to academic references (`Analysis/stage2_academic_references.md`) |
| 3 — Autonomy | specification ready |

## Quick start

```bash
php verify_all.php --stage=0 --production --log=logs/agenda.log --db=data/swarm.db
```
...
```bash
php agenda.php   # requires .deploy_ok marker (see DEPLOY.md)
```
...
├── Bee (energy lifecycle §2.1, heritable energy params §2.1-эво)
├── Search::find (CV→0, held-out, affine-shift §ЭКСП-012)
├── DiscoveryEngine (candidate pipeline)
├── GrammarMutator (spawn mutation + GRAMMAR-PROPAGATION weights)
├── Grammar (BASE_OPS + dynamic ops + B-атомы birth)

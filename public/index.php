...
if ($path === '/status') {
    ...
    reply([
        ...
        'laws' => $db->query('SELECT COUNT(*) FROM laws')
            ->fetchColumn(),
    ]);
}
...
if ($path === '/cross-domain') {
    $db = Database::get();
    $laws = $db->query('SELECT name,formula,cv,domain FROM laws ORDER BY domain,name')
        ->fetchAll();
    $ops = [
        '×' => [],
        '+' => [],
    ...
    foreach ($laws as $l) {
        ...
        foreach ($ops as $op => &$list) {
            if (str_contains($l['formula'], $op) && ! in_array($l['name'], $list)) {
                $list[] = $l['name'];
            }
        }
    }
...
if ($path === '/talk') {
    ...
    foreach ($words as $w) {
        $c = $onto->resolve($w);
        if (isset($onto->concepts[$c])) {
            $cs[] = $c;
        }
        $inf = $learner->query($c);
        if ($inf['facts_known'] || $inf['facts_inferred']) {
            $cs[] = $c;
        }
    }
    if (! $cs) {
        reply([
            'answer' => 'Не знаю. Научи: «X — это Y».',
            'cv' => 1.0,
        ]);
    }
    $cs = array_unique($cs);
    ...
    $cv = 1 - ($cov / count($cs));
    reply([
        'answer' => $cv == 0 ? 'Точно: ' . implode('; ', $lines) : 'Знаю: ' . implode('. ', $lines),
        'cv' => round($cv, 3),
        'covered' => $cov,
        'total' => count($cs),
    ]);

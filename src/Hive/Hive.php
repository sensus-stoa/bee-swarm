...
class Hive
{
    ...
    public function partialBirth(string $formula, float $cv, string $domain, float $cvMean): string|false
    {
        ...
        // mb_strlen: юникодные операторы (× = 2 байта, − = 3, ² = 2) раздувают
        // байтовую длину — символьная компрессия мерится символами (05.09).
        $birthOverhead = mb_strlen('(xBxx)') + 1; // минимальный контекст встраивания
        if (mb_strlen($formula) > 2 * $birthOverhead) {
            return false;
        }

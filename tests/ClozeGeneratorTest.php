...
class ClozeGeneratorTest extends TestCase
{
     ...
     */
    public function testRegistryStoresSentences(): void
    {
     ...
     */
    public function testGeneratesClozeTasks(): void
    {
     ...
     */
    public function testClozeTasksAreFalsifiable(): void
    {
        $tasks = $this->generateClozeTasks();
        $task = $tasks[0];
     ...
     */
    public function testErrorRateCv(): void
    {
        $tasks = $this->generateClozeTasks();
     ...
     */
    public function testRandomGuessHighError(): void
    {
        $tasks = $this->generateClozeTasks();
        $task = $tasks[0];
    ...
    private function generateClozeTasks(): array
    {
        ...
        for ($i = 0; $i < $this->registry->count(); $i++) {
            ...
            foreach ($ids as $pos => $targetId) {
                ...
                if (in_array($word, ['и', 'в', 'на', 'с', 'не', 'или', 'но', 'а', 'за', 'из', 'от', 'до'])) {
                    continue;
                }
                if (mb_strlen($word) < 2) {
                    continue;

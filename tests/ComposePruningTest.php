...
class ComposePruningTest extends TestCase
{
    ...
    public function test_compose_verified_on_correct_data(): void
    {
        // abs(sub) — |x-y|
        $law = ['name' => 'test_comp_ok', 'formula' => 'abs(sub(x0,x1))', 'cv' => 0.0, 'domain' => 'test_comp'];
    ...
    public function test_false_compose_rejected(): void
    {
        // sq(add) applied to |x-y| data → wrong
        $law = ['name' => 'test_comp_bad', 'formula' => 'sq(add(x0,x1))', 'cv' => 0.0, 'domain' => 'test_comp'];
        $newData = [[1.0, 3.0, 2.0], [5.0, 1.0, 4.0], [2.0, 2.0, 0.0]]; // |x-y|
    ...
    public function test_prune_compose_domain(): void
    {
        $db = Database::get();
        // Добавляем тестовые compose-законы
        $db->prepare("INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)")->execute(['test_c1', 'abs(sub(x0,x1))', 0.0, 'test_comp']);
        $db->prepare("INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)")->execute(['test_c2', 'sq(add(x0,x1))', 0.0, 'test_comp']);
        $db->prepare("INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)")->execute(['test_c3', 'mul(min(x0,x1))', 0.0, 'test_comp']);

        $countBefore = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test_comp'")->fetchColumn();
        ...
        $countAfter = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test_comp'")->fetchColumn();
    ...
    public function test_pruned_laws_not_revived(): void
    {
        $db = Database::get();
        $db->prepare("INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)")->execute(['test_dedup', 'sq(add(x0,x1))', 0.0, 'test_comp']);

        $newData = [[1.0, 3.0, 2.0], [5.0, 1.0, 4.0], [2.0, 2.0, 0.0], [3.0, 0.0, 3.0], [7.0, 2.0, 5.0]];
        $this->pruneDomain('test_comp', $newData, 0.1);

        // Проверяем что удалён
        $count = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test_comp'")->fetchColumn();
    ...
    {
        if (count($newData) < 2) return ['verified' => false, 'cv' => 9.99];
        $formula = $law['formula'];

        // Parse compose: outer(inner(x0,x1))
        if (!preg_match('/^(\w+)\((\w+)\(x0,x1\)\)$/', $formula, $m)) {
            return ['verified' => false, 'cv' => 9.99];
        }
        $outer = $m[1]; $inner = $m[2];
        ...
        foreach ($X as $row) {
            ...
            if ($v1 === null) return ['verified' => false, 'cv' => 9.99];
            $v2 = AtomRegistry::apply($outer, $v1);
            if ($v2 === null) return ['verified' => false, 'cv' => 9.99];
            $vec[] = $v2;
        }

        $cv = AtomRegistry::cv($vec, $y);
        return ['verified' => $cv < 0.1, 'cv' => $cv];
    }
    ...
    private function pruneDomain(string $domain, array $newData, float $threshold): int
    {
        ...
        foreach ($laws as $law) {
            ...
            if (!$result['verified'] && $result['cv'] > $threshold) {
                $db->prepare("DELETE FROM laws WHERE name = ? AND domain = ?")->execute([$law['name'], $domain]);
                $pruned++;
            }
        }
        return $pruned;

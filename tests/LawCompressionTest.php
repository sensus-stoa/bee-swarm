...
class LawCompressionTest extends TestCase
{
     ...
     */
    public function testClusterByOuterAtom(): void
    {
        $db = Database::get();
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c1', 'or(+)', 0.0, 'test_cmp']);
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c2', 'or(−)', 0.0, 'test_cmp']);
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c3', 'or(sq)', 0.0, 'test_cmp']);
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c4', 'or(max)', 0.0, 'test_cmp']);
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c5', 'sq(+)', 0.0, 'test_cmp']);
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c6', 'sq(−)', 0.0, 'test_cmp']);
     ...
     */
    public function testSmallClusterNotCompressed(): void
    {
        $db = Database::get();
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c1', 'abs(sub)', 0.0, 'test_cmp']);
        $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
            ->execute(['c2', 'abs(add)', 0.0, 'test_cmp']);
     ...
     */
    public function testCompressCluster(): void
    {
        $db = Database::get();
        for ($i = 0; $i < 5; $i++) {
            $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')
                ->execute(["c{$i}", "or(op{$i})", 0.0, 'test_cmp']);
        }

        $countBefore = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test_cmp'")
            ->fetchColumn();
        ...
        $countAfter = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test_cmp'")
            ->fetchColumn();
        $this->assertLessThan($countBefore, $countAfter, 'Laws decreased');

        // Проверяем что meta-law создан
        $meta = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test_cmp' AND name LIKE 'meta_%'")
            ->fetchColumn();
    ...
    private function compressClusters(string $domain): int
    {
        ...
        foreach ($clusters as $outer => $count) {
            if ($count >= 3) {
                ...
                $db->prepare('DELETE FROM laws WHERE domain = ? AND formula LIKE ?')
                    ->execute([$domain, "{$outer}(%"]);
                $db->prepare('INSERT INTO laws (name, formula, cv, domain) VALUES (?,?,?,?)')

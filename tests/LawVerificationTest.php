...
class LawVerificationTest extends TestCase
{
     ...
     */
    public function testVerifyTrueLaw(): void
    {
        // Закон: ADD = add(x0,x1)
        $law = [
            'name' => 'test_ADD',
     ...
     */
    public function testVerifyFalseLaw(): void
    {
     ...
     */
    public function testPruneFalseLaws(): void
    {
        ...
        $countBefore = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test'")
            ->fetchColumn();
        ...
        $countAfter = $db->query("SELECT COUNT(*) FROM laws WHERE domain = 'test'")
            ->fetchColumn();
        $this->assertEquals(1, $countAfter, 'One false law should be pruned');
        $this->assertEquals(1, $pruned, 'Should report 1 pruned law');

        // Уцелевший закон
        $survivor = $db->query("SELECT name FROM laws WHERE domain = 'test'")
            ->fetchColumn();
     ...
     */
    public function testUnknownFormulaSkipped(): void
    {
        $law = [
            'name' => 'test_unknown',
            'formula' => 'unknown_op(x0,x1)',
            'cv' => 0.0,
            'domain' => 'test',
        ];
        $newData = [[1.0, 2.0, 3.0]];
     ...
     */
    public function testInsufficientDataSkipped(): void
    {
        $law = [
            'name' => 'test_few',
            'formula' => 'add(x0,x1)',

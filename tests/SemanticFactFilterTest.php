...
class SemanticFactFilterTest extends TestCase
{
    ...
    {
        $db = Database::get();
        $before = (int) $db->query("SELECT COUNT(*) FROM knowledge_graph WHERE predicate='is_a'")
            ->fetchColumn();
        ...
        $after = (int) $db->query("SELECT COUNT(*) FROM knowledge_graph WHERE predicate='is_a'")
            ->fetchColumn();
     ...
     */
    public function testStopwordsFiltered(): void
    {
        $db = Database::get();
        $before = (int) $db->query("SELECT COUNT(*) FROM knowledge_graph WHERE predicate='is_a'")
            ->fetchColumn();
     ...
     */
    public function testValidWordsAccepted(): void
    {
        $db = Database::get();

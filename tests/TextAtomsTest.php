...
class TextAtomsTest extends TestCase
{
     ...
     */
    public function testContextReturnsSurroundingWords(): void
    {
     ...
     */
    public function testMatchDetectsWordInWindow(): void
    {
        $s = $this->registry->get(0);
        $ids = $s['token_ids'];
     ...
     */
    public function testComposeContextMatchPredictsClozeTarget(): void
    {
        $s = $this->registry->get(0);
        $ids = $s['token_ids'];
        ...
        $this->assertEquals(
            1.0,
            $this->matchInWindow($window, $ids[0]),
            'Сократ в контексте является → match=1'
        );
        $this->assertEquals(
            1.0,
            $this->matchInWindow($window, $ids[2]),
     ...
     */
    public function testNegativeMatch(): void
    {
        $s = $this->registry->get(0);
        $ids = $s['token_ids'];
        $window = $this->contextWindow($ids, 1, 1); // [Сократ, человеком]
        ...
        if (! $anotherS) {
            $this->markTestSkipped('Need 4+ sentences');
        }
        if ($anotherS) {
     ...
     */
    public function testClozeErrorRateZeroWithCorrectAtoms(): void
    {
